"""
CAUSAL_DETECTION (Detecting causal statements in financial text) task for Evalchemy.

This module implements the CAUSAL_DETECTION task from FLaME within the Evalchemy framework.
"""
...
class CausalDetectionBenchmark(BaseBenchmark):
    """CAUSAL_DETECTION benchmark for Evalchemy."""
    ...
    def __init__(
        ...
        **kwargs
        ...
        """Initialize CAUSAL_DETECTION benchmark."""
        super().__init__(logger=logger, system_instruction=system_instruction)
        self.max_new_tokens = max_new_tokens
        self.temperature = temperature
        self.debug = debug
        self.dataset_name = 'gtfintechlab/CausalDetection'
        self.split = 'test'
        
        # BIO tag mappings for CausalDetection (Cause, Effect)
        self.label_to_id = {
            "B-CAUSE": 0, "I-CAUSE": 1,
            "B-EFFECT": 2, "I-EFFECT": 3, 
            "O": 4
        }
        self.id_to_label = {v: k for k, v in self.label_to_id.items()}
        
    def load_dataset(self):
        """Load CAUSAL_DETECTION dataset with fallback to mock data."""
        try:
            ...
            self.logger.info(f"Loaded {len(dataset)} examples from CAUSAL_DETECTION dataset")
            return dataset
        except Exception as e:
            self.logger.warning(f"Failed to load real dataset: {e}")
            self.logger.info("Using mock CAUSAL_DETECTION dataset for testing")
            
            # Use mock data
            from .mock_dataset import get_mock_causal_detection_data
            mock_data = get_mock_causal_detection_data()
    ...
    def create_prompt(self, example: Dict[str, Any]) -> str:
        """Create CausalDetection sequence labeling prompt."""
        tokens = example.get('tokens', [])
        
        # Format tokens as a numbered list
        token_list = "\n".join([f"{i+1}. {token}" for i, token in enumerate(tokens)])
        
        prompt = f"""Discard all previous instructions. Behave like you are an expert causal relationship detection system.

Your task is to identify cause and effect phrases in the following tokenized financial text.
...
- O: Other (not part of cause or effect)
...
..."""
    ...
    def generate_responses(self, model: LM) -> Dict[str, Any]:
        ...
        for idx, example in enumerate(dataset):
            ...
            instance.task_name = "flame_causal_detection"
    ...
    def parse_output(self, output: str, example: Dict[str, Any]) -> List[int]:
        ...
        for i, line in enumerate(lines):
            ...
            # Extract label from line (format: "1. B-CAUSE" or "B-CAUSE")
